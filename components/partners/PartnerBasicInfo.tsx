    ...
    <div className="bg-white rounded-xl border border-gray-200 p-6 mb-4">
      ...
      <div className="mb-4">
        ...
        {editing ? (
          <div>
            <textarea
              className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-blue-400 font-mono"
              rows={5}
            ...
            <p className="text-[11px] text-gray-400 mt-1">
              支持 Markdown：<code className="bg-gray-100 px-1 rounded">**加粗**</code>、
              <code className="bg-gray-100 px-1 rounded">*斜体*</code>、
              <code className="bg-gray-100 px-1 rounded">- 列表</code>、空行分段
            </p>
          </div>
        ) : (
          <div className={description ? 'prose prose-sm max-w-none text-gray-700' : ''}>
            {description ? (
              <ReactMarkdown>{description}</ReactMarkdown>
            ) : (
              <span className="text-gray-300 text-sm">
                {isAdmin ? '点击"编辑"添加背景介绍' : '暂无'}
              </span>
            )}
          </div>
        )}
